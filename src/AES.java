...
public class AES {
    ...
    /**
     * AES constructor for which there's  nothing to do.
     */
    public AES() {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            {
                key = (new BufferedReader(new FileReader(args[1]))).readLine();
                if (key.length() * 4 != 128) {
                    throw new Exception("Error: Attemping to use a " + key.length() * 4 + "-bit key with AES-128"); //Unmatched key argument and key input
                }
                input = new BufferedReader(new FileReader(args[2]));
                ftw += args[2];
            } else {
                key = (new BufferedReader(new FileReader(args[3]))).readLine();
                if (key.length() * 4 != Integer.parseInt(args[2])) {
                    throw new Exception("Error: Attemping to use a " + key.length() * 4 + "-bit key with AES-" + args[2]);
                }
                input = new BufferedReader(new FileReader(args[4]));
                ftw += args[4]; //ftw => File To Write
            ...
            if (args[0].equalsIgnoreCase("e")) {
                ...
                while (line != null) {
                    if (aes.validline(line)) {
                        ...
                        aes.addRoundKey(bytematrix, aes.subKey(keymatrix, 0)); //Starts the addRoundKey with the first part of Key Expansion
                        for (int i = 1; i < numRounds; i++) {
                            aes.subBytes(bytematrix); //implements the Sub-Bytes subroutine.
                            aes.shiftRows(bytematrix); //implements Shift-Rows subroutine.
                            aes.mixColumns(bytematrix);
